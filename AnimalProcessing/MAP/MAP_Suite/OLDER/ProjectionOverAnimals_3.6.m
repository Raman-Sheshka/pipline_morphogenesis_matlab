...
newBackup = struct();
gridInfoListExt = [gridInfoList ; 'AreaRatios']; % adding "AreaRatios" to gridInfoList defined in AIA_MultiOperation (3.6)
% gridInfoListExt = {'xywh' ; 'size' ; 'Lcentroids' ; 'overlap' ; 'color' ; 'centroids' ; 'ULCs' ; 'lineWidth' ; 'TimeArray' ; 'FrameArray' ;...
%             'RConds' ; 'Macrocaetes' ; 'AreaRatios' ; 'AreaRatios_VM';...
...
for i=1:size(gridInfoListExt,1)
    if isfield(BACKUP,gridInfoListExt{i})
        eval(['newBackup.' gridInfoListExt{i}  '= BACKUP.' gridInfoListExt{i} ';']);
    end
end
...
notProcessed = false(nQname,1); % 3.4
notFound = false(nQname,1); % 3.5
for t = 1:length(tagTensor)
    ...
    for i = 1:nQname
        fprintf(['    Processing ' Qname{i} '.' tagTensor{t} '(' uQname ') ...']);
        
        if ~isfield(BACKUP,Qname{i}) % 3.5
            fprintf([' SKIPPED because ' Qname{i} ' was NOT found in AOA_backup!\n']);
            notFound(i) = true;
            continue
        else
            % loading A
            ...
            % skipping projection when A NOT a tensor (3.4) AND saving A and A_Smap in backup (3.5)
            if size(A,3) < 4
                fprintf([' SKIPPED because ' Qname{i} ' is not a tensor!\n']);
                notProcessed(i) = true;
                % storing UNTOUCHED quantity (and its Smap) for convenience in LTA (3.5)
                newBackup.(Qname{i}) = A;
                A_Smap = eval(['BACKUP.' Qname{i} '_Smap;']);
                newBackup.([Qname{i} '_Smap']) = A_Smap;
                continue
            end
            ...
            % projection function
            AdotB = ScalarProductFunction(A,sizedBu);
            ...
            eval(['newBackup.' Qname{i} 'dot' uQname '_' tagTensor{t} ' = AdotB ;']);
            fprintf(' Done\n');
        end
    end
end
...
%             pathbackup = [RAW.gridAnimalFolder filesep 'Backups' filesep Pname '_LGrid_' Animal '_' num2str(framePlot,['%0' num2str(RAW.nDigits) 'd']) '.mat'];
        ...
        for n = 1:nQname
            ...
            % skipping plot when non-tensor quantity was NOT processed (3.4)
            if notProcessed(n)
                disp([PlotQname ' is irrelevant and was not plotted!'])
                continue
            elseif notFound(n) % 3.5
                disp([PlotQname ' could not be plotted due ' Qname{n} ' to not found!'])
                continue
            end
            
            disp(['Plotting ' PlotQname '...']); % 3.4
            ...
            if PLOT.print
                thisFilename = ['POA_' uAnimal '_' PlotQname '_' tagProjectionTime '_' timeStart 'to' timeEnd fullTag '_sr=' num2str(QsrPOA) imageExtension]; %  added fullTag (3.2)
%                 thisFilename = ['POA_' uAnimal '_' PlotQname '_' tagProjectionTime '_' BACKUP.TimeArray{t,1} 'to' BACKUP.TimeArray{t,2} fullTag '_sr=' num2str(QsrPOA) imageExtension]; %  added fullTag (3.2)
