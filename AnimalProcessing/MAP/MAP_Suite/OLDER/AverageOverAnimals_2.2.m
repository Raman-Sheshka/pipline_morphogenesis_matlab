...
for n = 1:size(allAnimals,1) % for all the animal
...
for m = 1:size(list,1)
    ...
    if strncmp(list{m},'AreaRatios',10)
        total_AreaRatios = zeros(Ltoty, Ltotx, Lz, Lt, size(avgAnimals,1)); % specific case for AreaRation
    elseif strcmp(list{m},'RConds')
        total_RConds = zeros(Ltoty, Ltotx, Lz, Lt, size(avgAnimals,1)); % specific case for RConds
    end
...
eval(['AOA_backup.TimeArray  = GRID_' PnameAOA '_' avgAnimals{1} '.TimeArray;']);
eval(['AOA_backup.FrameArray = GRID_' PnameAOA '_' avgAnimals{1} '.FrameArray;']);
eval(['AOA_stdMAP.TimeArray  = GRID_' PnameAOA '_' avgAnimals{1} '.TimeArray;']);  % (v2) in progress add-on for biological variability estimation
eval(['AOA_stdMAP.FrameArray = GRID_' PnameAOA '_' avgAnimals{1} '.FrameArray;']); % (v2) in progress add-on for biological variability estimation
...
%         DISPLAY.time = [ AOA_backup.TimeArray{i,1} ' - ' AOA_backup.TimeArray{i,2} ];
        ...
        for j = 1:length(Qname)
            ...
            Qsr       = eval(['sr_' PnameAOA '{idx}']);
            Qscalebar = eval(['srbar_' PnameAOA '(idx)']);
            Qunits    = eval(['allUnits_' PnameAOA '{idx}']);
            Qcolor    = eval(['allColors_' PnameAOA '{idx}']);
            QKillTr   = eval(['killtrace_' PnameAOA '(idx)']);
