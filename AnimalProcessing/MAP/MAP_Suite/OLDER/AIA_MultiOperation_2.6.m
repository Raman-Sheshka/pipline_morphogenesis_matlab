...
COQ = 1;               % Calcul Over Quantities                         
POA = 1;               % Projection Over Animal
...
PathName = 'D:\BigMovies\AOA_26h00_DEBUG';
...
% PathName = 'D:\BigMovies\AOA_WT_test';
% AIAFolderName = 'C:\Users\Boris\ownCloud\Matlab'; % path to the folder containing the AIA_info_X



%% Main Common Parameters %%

%%% name of average OR actual animal name (BIG1, TRBL4...) when processing single animal
% Animal ='meanWT_test';
% Animal ='meanWT';
% Animal ='meanTRBL';
% Animal ='meanWT_grid26h';
% Animal = 'meanTRBL_grid26h';        
Animal = 'BIGwt2';
% Animal = 'R23';
% Animal = 'TRBL8';

% animalTimeWidthAll   = 20;
% animalTimeOverlapAll = 0;  % to make a plot at every single frame

animalTimeWidthAll   = [20 ; 2];
animalTimeOverlapAll = [0 ; 0.96];  % to make a plot at every single frame
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Qname = {'dnA';'dnD'};  % Quantity names 'Rho','M','CDcad','EG',dnA...
% Qname = {'EG';'S'};  % Quantity names 'Rho','M','CDcad','EG',dnA...
Qname = {'EG';'dnA';'dnD';'S'};  % Quantity names 'Rho','M','CDcad','EG',dnA...
% Qname = {'EG';'dnA';'ES'};  % Quantity names 'Rho','M','CDcad','EG',dnA...
...
uOname = 'AOA';      % Operation name (AOA,DOA...) that produced the tensor ONTO WHICH we project and that led to "uAnimal"
% (overridden by "empty" when single animal processing"
...
% PLOT.boxSize = 128*[1 1]*0.322/0.216;           % box size of the average grid
...
plotAll = [1 ; 0];                  % plot the selected quantities (Qname) FOR EACH "animalTimeWidthAll" (2.5)
...
OnameDOA = 'AOA'; % Operation name WHAT ELSE THAN AOA CAN IT BE???????????????????????????????????????????? => REMOVE
% NB: single animal do NOT work because one must build a common grid before subtraction

% Animal 1 minus Animal 2: the name of the output animal will be "A1-A2"
deltaAnimals  = { 'meanWT_grid26h' ; 'meanTRBL_grid26h'};
...
CalculType = {'plus'}; % Ex: {'minus'; 'plus' ;'minus'}
...
% NB: all 3 'd','i','do' are ALWAYS calculated though

% project this:
OnamePOA = 'AOA';      % Operation name (AOA or DOA) (overridden by "empty" when single animal processing)
QunitsPOA = 'h^{-1}';
QsrPOA    = 50*10^1;
QsbPOA    = 0.1;
...
% OnameLTA = 'AOA';         % Operation name (AOA, DOA, POA...)
DOAplot = false; % NB: one could ONLY USE OnamePOA !!!!!!!!!!!!!!!!!!!!!!!

% QnameLTA = {'dnA','dnD'}; % list of quantities to be PLOTTED
% Qfactor =  [  1    0.1  ]; % to be able to plot different type of data (2.3)
%  
% QnameLTA = {'SdotEG','EGdotEG'}; % list of quantities to be PLOTTED
% Qfactor =  [ 7.5e-1      1]; % to be able to plot different type of data (2.3)

QnameLTA = {'dnA','dnD','SdotEG','EGdotEG', 'dnAplusdnD'}; % list of quantities to be PLOTTED
Qfactor =  [  1    0.1   7.5e-1     1           0.1 ]; % to be able to plot different type of data (2.3)

% QnameLTA = {'dnA','dnD','SdotEG','EGdotEG'}; % list of quantities to be PLOTTED
% Qfactor =  [  1    0.1   7.5e-1     1]; % to be able to plot different type of data (2.3)
...
% Defining AOAtag accordingly (moved here 2.7)
AOAtag = 'AOA_'; % default (2.6)
if singleAnimal
    AOAtag = '';
end
...
% NB: AOA doesn't care of value of parameter "singleAnimal" and can still process individual animals.
% Same with DOA that can ONLY take AOA outputs to proceed (and in that case nAllAnimals >= 2 anyway)
...
gridInfoList = {'xywh' ; 'size' ; 'Lcentroids' ; 'overlap' ; 'color' ; 'centroids' ; 'ULCs' ; 'lineWidth' ; 'TimeArray' ; 'FrameArray' ;...
                'fullImage' ; 'lineWidth' ; 'coordinates'; 'Macrocaetes' ; 'REG' ; 'FULL'};
...
nTimeAvg = length(animalTimeWidthAll);
for p = 1:nTimeAvg
    
    %%% Defining quantities for THIS time averaging (2.5)%%
    TIME.animalTimeOverlap = animalTimeOverlapAll(p);
    TIME.animalTimeWidth   = animalTimeWidthAll(p);
    PLOT.plot = plotAll(p);                      % plot the selected quantities (Qname)
    

    %% AOA - Advanced %%
    
    if AOA && nAllAnimals > 1       % prevents AOA executions if nAllAnimals = 1 (since useless) (2.6)
        
        fprintf('\nStarting AOA processing ...\n')
        ...
        for n = 1:nAllAnimals
            ...
            % NB: as of 2.6+, "SingleAnimalLoader" updates each animal's AOT backup by adding common "AreaRatios"
            ...
            if ismember( allAnimals{n}, avgAnimals )
                ...
                % TO BE ADDED: AreaRatios_VM
%                 [~, thisAOTbackup] = AreaRatiosAssigner(thisAOTbackup); % 2.6
%                 thisAOTbackup.AreaRatios = AreaRatios;
        ...
        % Output Path (2.6)
        OutputPathName = [PathName filesep AOAtag Animal '_' num2str(TIME.animalTimeWidth) 'h_olap_' num2str(TIME.animalTimeOverlap) ];
%         OutputPathName = [PathName filesep 'AOA_' Animal '_' num2str(TIME.animalTimeWidth) 'h_olap_' num2str(TIME.animalTimeOverlap) ];
        ...
        disp('Done ... !')
    elseif AOA
        disp('*** AOA was skipped because of SINGLE animal processing! ***')
    end
    
    
    %% DOA - Advanced %%
    
    DOAname = [deltaAnimals{1} '-' deltaAnimals{2}]; % moved out of if (2.5)
    
    if DOA && ~singleAnimal % directly skipping when singleAnimal (2.6)
        
        fprintf('\nStarting DOA processing...\n')
        disp('Load corresponding backup...')
        
        LoadedBackup = cell(1,2);
        if isempty(OnameDOA)
            
            disp('ERROR: DOA cannot make difference of single animals because their respective grids do NOT match.')
            disp('TIPS : Ru AOA on those single animals first, then rerun DOA on their obtained "averages".')
            
        else
            
            % animal 1
            disp([PathName filesep OnameDOA '_' deltaAnimals{1} '_' num2str(TIME.animalTimeWidth) 'h_olap_' num2str(TIME.animalTimeOverlap) filesep OnameDOA '_backup']);
            LoadedBackup{1} = load( [PathName filesep OnameDOA '_' deltaAnimals{1} '_' num2str(TIME.animalTimeWidth) 'h_olap_' num2str(TIME.animalTimeOverlap) filesep OnameDOA '_backup'] );
            
            % animal 2
            disp([PathName filesep OnameDOA '_' deltaAnimals{2} '_' num2str(TIME.animalTimeWidth) 'h_olap_' num2str(TIME.animalTimeOverlap) filesep OnameDOA '_backup']);
            LoadedBackup{2} = load( [PathName filesep OnameDOA '_' deltaAnimals{2} '_' num2str(TIME.animalTimeWidth) 'h_olap_' num2str(TIME.animalTimeOverlap) filesep OnameDOA '_backup'] );
        end
                
        % output folder path
        OutputFolderName = [PathName filesep 'DOA_' DOAname '_' num2str(TIME.animalTimeWidth) 'h_olap_' num2str(TIME.animalTimeOverlap) ];
        ...
        disp('Done ... !')
    elseif DOA
        disp('*** DOA was skipped because of SINGLE animal processing! ***')
    end
    ...
    if COQ
        ...
        fprintf('\nStarting COQ processing ...\n')
        disp('Loading corresponding backup ...')
        if isempty(OnameCOQ)            % updated (2.6)
            PLOT.significance = 0;
            PLOT.macrocaetes = 0;
            [~, backupPathList, RAW] = SingleAnimalLoader(AIAFolderName, Animal, TIME,  true );
            BACKUP = load( backupPathList );
            [OutputPath, backupPathName,~]  = fileparts(backupPathList);
            backupName = ['GRID_AOT_' Animal];
            % NB: AOT backups are directly appended with new values quantities here
        else
    ...
    if POA
        ...
        if isempty(OnamePOA)
            ...
            % 
            animalName = Animal;
            if strcmp(OnamePOA,'DOA')
                animalName = DOAname;
            end
            
            animalProjectionFolder = [OnamePOA  '_' animalName  '_' num2str(TIME.animalTimeWidth)  'h_olap_' num2str(TIME.animalTimeOverlap) ]; % 3.6
%             animalProjectionFolder = [OnamePOA  '_' Animal  '_' num2str(TIME.animalTimeWidth)  'h_olap_' num2str(TIME.animalTimeOverlap) ];
            ...
            disp( ['Loading backup FOR projection...' uQname '(from ' uOname ')'] );            % mod 3.6
            thisFilename = [PathName filesep animalProjectorFolder filesep uOname '_backup'];   % "uOname" replaced "OnamePOA" (3.6)
%             thisFilename = [PathName filesep animalProjectorFolder filesep OnamePOA '_backup'];
    ...
    %% LTA - Advanced (overhaul 2.6) %%
    
    if LTA && TIME.animalTimeOverlap == 0.96 % mod 2.5
        
        fprintf('\nStarting LTA processing ...\n')
        
        tagTensor = {''; '_d'; '_i'; '_do'}; % added '' to also plot scalar quantities that have been copied to POA backups (although not projected, 2.4)
        LTAbackup = struct();
        tagProjection = ['_' strrep(num2str(uTimeWidth),'.','') '_' strrep(num2str(uTimeOverlap),'.','')];
        
        if ~DOAplot % AOA case
            
            fprintf('\nCreating LTA backup gathering AOA & POA ones ...')
            
            animalName = Animal;
            
            InputPathName = [PathName filesep AOAtag animalName '_' num2str(TIME.animalTimeWidth) 'h_olap_' num2str(TIME.animalTimeOverlap)]; % AOA & POA
            backupPathName = 'AOA_backup.mat';
            backupPathListAOA = [InputPathName filesep backupPathName];
            
            LTAbackupPath = [InputPathName  filesep 'LTA_backup.mat'];
            save(LTAbackupPath,'-struct', 'LTAbackup'); % REinitializes backup that will JUST be used for LTA plots
            
            if exist(backupPathListAOA, 'file')
                BACKUP_AOA = load( backupPathListAOA );
                save(LTAbackupPath,'-struct', 'BACKUP_AOA','-append');
            end

        elseif DOAplot && ~singleAnimal % DOA case
            
            fprintf('\nCreating LTA backup gathering DOA & POA ones...')
            
            animalName = DOAname;
  
            InputPathName = [PathName filesep 'DOA_' animalName '_' num2str(TIME.animalTimeWidth) 'h_olap_' num2str(TIME.animalTimeOverlap)]; % 2.5
            backupPathName = 'DOA_backup.mat';
            backupPathListDOA = [InputPathName filesep backupPathName];
            
            LTAbackupPath = [InputPathName  filesep 'LTA_backup.mat'];
            save(LTAbackupPath,'-struct', 'LTAbackup'); % REinitializes backup that will JUST be used for LTA plots
            
            if exist(backupPathListDOA, 'file')
                BACKUP_DOA = load( backupPathListDOA );
                save(LTAbackupPath,'-struct', 'BACKUP_DOA','-append');
            end
            
        end
        
        backupPathName = ['POA_' uAnimal tagProjection '.mat'];
        backupPathListPOA = [InputPathName filesep backupPathName];
        % NB: "InputPathName" depends on "DOAplot" value!!!
        
        if exist(backupPathListPOA, 'file')
            BACKUP_POA = load( backupPathListPOA );
            save(LTAbackupPath,'-struct', 'BACKUP_POA','-append');
        end
        
        fprintf('Done\n')
        
        OutputPath = fileparts(LTAbackupPath);
        BACKUP = load( LTAbackupPath );
...
%                 [~, backupPathList, RAW] = SingleAnimalLoader(AIAFolderName, Animal, TIME,  true ); % 2.7
...
%                 InputPathName = [PathName filesep 'DOA_' DOAname '_' num2str(TIME.animalTimeWidth) 'h_olap_' num2str(TIME.animalTimeOverlap)]; % 2.5
%             else
...
% 06/02/2017: 2.6 (Boris)
% - automatic detection of "singleAnimal"
% - prevents AOA executions if nAllAnimals = 1 (since it is useless)
% - now updates each animal's AOT backup with commong "AreaRatios" in "SingleAnimalLoader"
% - when processing a single animal (nAllAnimals = 1), EVERY outputs are now stored in a SINGLE COMMON folder ("BIGwt2_20h_olap_0"
% for instance) instead of creating "AOA_BIGwt2..." folder for AOA (only created if (nAllAnimals > 1), "BIGwt2_20h_olap_0" for POA,
% and in the original animal folder for COQ, thereby mimicking what is done when averaging a set of animals.

% 02/02/2017: 2.5 (Boris)
% - now runs several time averages at once (typically, [20h,0] and [2,0.96])
% - accordingly introduced parameter "plotAll" to be able to only plot one time average
% - adjustments for DOA and COQ updates
% - COQ: now processes several operations A+/-B at the same time

% 26/01/2017: 2.4 (Boris)
% - simplifying LTA
