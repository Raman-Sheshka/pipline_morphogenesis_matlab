...
if PLOT.makeItFull
    ...
    newOy = newOy + extraImageY;
    meanYmid = newOy - meanDeltaYmid;           % meanYmid update 
    sizeImageX = ceil(sizeImageX*1.05); % 5% leeway because of gridULCx starting a bit further from left border (2.6)
    sizeImageY = sizeImageY*2;
    Qimage = ones([sizeImageY , sizeImageX]);
    gridSize = GRID.size;                       % gets original grid size that will be kept
    % since using gridSize to keep previous size, need to determine grid ew ULC in extended image
%     gridULCx = PLOT.boxSize(1) * (1 - gridOverlap) - PLOT.boxSize(1)/2;
%     gridULCy = PLOT.boxSize(2) * (1 - gridOverlap) - PLOT.boxSize(2)/2 + extraImageY;
    gridULCx = PLOT.boxSize(1)/2;
    gridULCy = PLOT.boxSize(2)/2 + extraImageY;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    GRID = GridMaker([sizeImageY sizeImageX], PLOT.boxSize, [gridULCx gridULCy], gridSize , gridColor, gridLineWidth, gridOverlap);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    GRID.xywh = [newOx newOy GRID.xywh(3) GRID.xywh(4)];
    macrocaetes(1,:) = macrocaetes(1,:) + newOx;
    macrocaetes(2,:) = macrocaetes(2,:) + newOy;
    % filling coordinates for 8 flipped macrocaetes:
    macrocaetes(1,9:16) = macrocaetes(1,1:8);
    macrocaetes(2,9:16) = 2*meanYmid - macrocaetes(2,1:8); % (yMacro + yMacroFlipped)/2 = meanYmid => yMacroFlipped = ...
else
    macrocaetes(1,:) = macrocaetes(1,:) + newOx;
    macrocaetes(2,:) = macrocaetes(2,:) + newOy;
end
...
% Adding sizeImageX&Y to avoid having to rebuild it later (2.6)
AOA_backup.sizeImageX = sizeImageX;
AOA_backup.sizeImageY = sizeImageY;
AOA_backup.yMid = meanYmid; % 2.6

%%% Save AOA mean backup
if ~exist(OutputPathName,'dir')
    mkdir(OutputPathName); 
end
save([OutputPathName filesep 'AOA_stdMAP' fullTag '.mat'], 'AOA_stdMAP'); % added fullTag (2.6)

thisFilename = [OutputPathName filesep 'AOA_backup' fullTag '.mat']; % added fullTag (2.6)
...
if ~isempty(Qname) && PLOT.plot
    ...
    AOA_backup = load([OutputPathName filesep 'AOA_backup' fullTag]); % added fullTag (2.6)
