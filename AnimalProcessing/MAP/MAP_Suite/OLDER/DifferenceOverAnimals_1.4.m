    ...
    for t = 1:Lt
        for j=1:length(Qname)
            ...
            if PLOT.Qparameters
                ...
                if idx > 0
                    Qcolor  = eval(['allColors_' PnameDOA '{idx}']);
                    Qunits  = eval(['allUnits_' PnameDOA '{idx}']);
                    Qsr     = eval(['sr_' PnameDOA '{idx}']);
                    Qsb     = eval(['srbar_' PnameDOA '(idx)']);
                    QKillTr = eval(['killtrace_' PnameDOA '(idx)']);
                end
            end
            eval(['GRID.AreaRatios = GRID.AreaRatios_' PnameDOA ';']);
...
%                     stdT = eval( [OnameDOA '_stdMAP.' Qname{j} '_std;'] );
