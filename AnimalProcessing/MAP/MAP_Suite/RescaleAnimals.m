...
for a = 1:nAvgAnimals
    ...
    aName = avgAnimals{a};
    eval(['SAP_info_' aName]); % SAPcall or MAPcall already "true" at this point (4.2)
%     SAP_info_file = [AIAFolderName filesep 'SAP_info_' aName '.m' ];
%     if exist(SAP_info_file,'file')
%         run(SAP_info_file);
%     else
%         disp(['WARNING: "SAP_info_' aName '.m" not found in specified location! Trying Matlab saved paths...'])
...
% 06/07/2018: 4.3 (Stephane)
% - small fix on color picking for plotting

% 27/06/2018: 4.2 (Boris)
% - now always calling directly animal's "SAP_info_" without defining the folder containing the SAP_info files.

% 16/05/2018: 4.1 (Boris)
% - "AIA_" became "SAP_"
% - stopped defining "pathFolderSR" that is now defined in "SAP_parameters"
