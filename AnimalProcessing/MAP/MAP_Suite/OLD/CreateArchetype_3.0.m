% CreateArchetype
%
% /!\ NOW OBSOLETE BECAUSE INCORPORATED INTO "RescaleAnimals" 4.0+ /!\
%
% This program gathers the positions of different landmarks clicked in
% different movies and uses them to create an archetype.
%
...
if ~allHalfNotumIdenticalTF
    disp('ERROR in "CreateArchetype": all animals listed in "avgAnimals" must correspond to same "halfNotum"!')
    return
end
...
%   useMidPointTF = true;  
%   fprintf('warning: Mid landmarks will be used for origin calculation.\n');
% end
%--------------------------------------------------------------------------
...
oDataMidline = dataMidLin - repmat(originY,[3 1]);             % (pxl)
...
stdAnimals = [nanstd(Xall,0,2) nanstd(Yall,0,2)];
...
rescalingOutput(2:3,1)     = [{'Ox'}; {'Oy'}];              % 3.0
rescalingOutput(2:3,2:end) = num2cell([originX ; originY]);
rescalingOutput(4:5,1)     = [{'Xfactor'}; {'Yfactor'}];    % 3.0
rescalingOutput(4:5,2:end) = num2cell([scalingFactorX ; scalingFactorY]);
rescalingOutput(6:7,1)     = [{'yML'}; {'xNK'}];    % 3.0
rescalingOutput(6:7,2:end) = num2cell([dataMidLin(3,:) ; dataNeck(3,:)]);
rescalingOutput(8,1)       = {'scale1D'};
rescalingOutput(8,2:end)   = num2cell(scale1Dmat(1,:)); % 3.0
% write output in .mat and .txt format
save([pathFolderCA filesep 'rescalingOutput' originTypeTag],'rescalingOutput');
dlmcell([pathFolderCA filesep 'rescalingOutput' originTypeTag '.txt'],rescalingOutput,' ')
...
% write output in .mat and .txt format
save([pathFolderCA filesep 'stdOutput' originTypeTag],'stdOutput');
dlmcell([pathFolderCA filesep 'stdOutput' originTypeTag '.txt'],stdOutput,' ')
...
for a = 1:nAvgAnimals
    % color code
    color = cmap(round(factor * a),:);
    % scaling
    midlineVectorY = [oDataMidline(a) oDataMidline(a)] .* scale1Dmat(1,a); % (um)
    neckVectorX    = [oDataNeck(a) oDataNeck(a)]     .* scale1Dmat(1,a); % (um)
    macroVectorX   = oDataMacroX(:,a)                .* scale1Dmat(1,a); % (um)
    macroVectorY   = oDataMacroY(:,a)                .* scale1Dmat(1,a); % (um)
    % midline plot
...
print('-dpng', '-r400', [pathFolderCA filesep 'raw_animals_vs_archetype' originTypeTag '.png']); % 1.1
if makeSVG
    plot2svg([pathFolderCA filesep 'raw_animals_vs_archetype' originTypeTag '.svg'], figure(1), 'png'); % 1.1
end
close
...
for a = 1:nAvgAnimals
    ...
    midlineVectorY = [oDataMidline(a) oDataMidline(a)] .* scalingFactorY(a) .* scale1Dmat(1,a); % (um)
    neckVectorX    = [oDataNeck(a)   oDataNeck(a)]   .* scalingFactorX(a) .* scale1Dmat(1,a); % (um), use of "scalingFactorX" (not Y) (3.0)
    macroVectorX = scalingFactorX(a) .* oDataMacroX(:,a) .* scale1Dmat(1,a); % (um)
    macroVectorY = scalingFactorY(a) .* oDataMacroY(:,a) .* scale1Dmat(1,a); % (um)
    % mideline plot
    line([Xmin Xmax], midlineVectorY, 'LineStyle', ':', 'Color', color, 'LineWidth', 2)
    % macrochaete plot
    scatter(macroVectorX, macroVectorY, 20, color, 'LineWidth', 2)
...
print('-dpng', '-r400', [pathFolderCA filesep 'rescaled_animals_vs_archetype' originTypeTag '.png']); % 1.1
if makeSVG
    plot2svg([pathFolderCA filesep 'rescaled_animals_vs_archetype' originTypeTag '.svg'], figure(1), 'png'); % 1.1
end
close
...
txtFilename = [today '_CA_' version '.txt'];
...
dlmcell([pathFolderCA filesep txtFilename], parameterCell,' ');
...
[Full, Half] = CreateSymmetricArchetype(moviesA(1:end-1,1),moviesA(1:end-1,2), ... 
                                        moviesA(end,:), halfNotum, nMacroMin, useMidPointTF);
% Full.scale1Dmat = scale1Dmat(1,:);
% Half.scale1Dmat = scale1Dmat(1,:);
...
print('-dpng', '-r400', [pathFolderCA filesep 'full_symetric_archetype' originTypeTag '.png']); % 1.1
if makeSVG
    plot2svg([pathFolderCA filesep 'full_symetric_archetype' originTypeTag '.svg'], figure(3), 'png'); % 1.1
end
close
...
print('-dpng', '-r400', [pathFolderCA filesep 'half_symetric_archetype' originTypeTag '.png']); % 1.1
if makeSVG
    plot2svg([pathFolderCA filesep 'half_symetric_archetype' originTypeTag '.svg'], figure(3), 'png'); % 1.1
end
close

% save archetype
save([pathFolderCA filesep 'FullArchetype'],'-struct','Full');
save([pathFolderCA filesep 'HalfArchetype'],'-struct','Half');
