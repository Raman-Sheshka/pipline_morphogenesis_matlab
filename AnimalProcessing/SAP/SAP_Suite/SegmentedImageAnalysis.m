...
if matlabSIA == 0     % 3.6 set off all stuff not compatible with C++ exe
    SIAboxMode = 0;
    noStatistics = 1;
    reloadBox = 0;
end
    ...
    if ~exist(fnBackupFile,'file') % checks NON-existence of backup about to be generated BEFORE actual execution (2.14)
        ...
        if matlabSIA
            ...
            if SIAboxMode == 0
                ...
                skeletonPixelsListPath = [trackingFolder filesep 'skeletonPixelsList_' num2str(n) '.txt'];  % 3.5
                ...
                if exist(skeletonPixelsListPath,'file') % 2.12
                    ...
                    tic
                    spmd
                        CDindSkel = codistributed(skelIndicesM, codistributor1d(1));    % codistributes "indSkel": slices it to spread it evenly on the "nlabs" labs
                        LindSkel = getLocalPart(CDindSkel);                             % part of "indSkel" on this lab L
                ...
                %%% Defines "cellCategoryTags" (3.8)
                cellCategoryTags = NaN(nCells,1);
                cellCategoryTags(coreRNs) = 0;
                cellCategoryTags(FLRNs) = 1;
                cellCategoryTags(borderRNs) = 2;
                
%                 %%% Buils structure "cellCATEGORIES":
...
%                 %%% Start building allVertexCATEGORIES:
...
%                 allVertexCATEGORIES.fourVertices = fourVertices;
                        ...
                        for j = 1:nHigherNeighbors
                            ...
                            if nVerticesInvolved <= 2 % (REGULAR CASE nVerticesInvolved = 2)
                                ...
                                for r = 1:nRawImages
                                    ...
                                    rawImage = evalin('base',['rawImage_' filenameRawMod{r}]);
                                    sideIntensities = evalin('base',['sideIntensities' signalName{r}]); % use of "signalName" (3.0)
                                        ...
                                        while nNextVertex < nPthPARTVerticesInvolved
                                            ...
                                            if deltaLoc > 1 || (deltaLoc == 1 && vertexIndex == firstVertexIndex) || (deltaLoc == 1 && nextVertexIndex == lastVertexIndex)
                                                ...
                                                for r = 1:nRawImages
                                                    % Name this image variables:
                                                    rawImage = evalin('base',['rawImage_' filenameRawMod{r}]);
                                                    sideIntensities = evalin('base',['sideIntensities' signalName{r}]); % using signalName (3.0)
                                                    % Calculate mean intensity:
                ...
                for r = 1:nRawImages
                    ...
                    rawImage = evalin('base',['rawImage_' filenameRawMod{r}]);                           % Name this image variables:
                ...
                sideCATEGORIES = SortSides(sideNumbers, sideCells, cellCategoryTags); % 3.7
%                 sideCATEGORIES = SortSides(sideNumbers, sideCells, cellCATEGORIES);
                ...
                for i = 1:nCells
                    ...
                    %%% Texture calculation:
                    [Mi, ithCellnLinks] = CalculateTexture(i, cellCategoryTags, cellNeighbors, cellVertices, cellXYs, fourVertices); % 3.7
%                     [Mi, ithCellnLinks] = CalculateTexture(i, cellCATEGORIES, cellNeighbors, cellVertices, cellXYs, fourVertices);
                    ...
                    for r = 1:nRawImages
                        ...
                        sideIntensities = evalin('base',['sideIntensities' signalName{r}]); % use of "signalName" (3.0)
                        cellSideIntensityDisorders = evalin('base',['cellSideIntensityDisorders' signalName{r}]);
                        ...
                        cellPolarityModes = evalin('base',['cellPolarityModes' signalName{r}]); % use of "signalName" (3.0)
                        rawImage = evalin('base',['rawImage_' filenameRawMod{r}]);
                        % Removal of cell mean background intensity "A0_BG" (calculated over cell i sides):
                        % Loading:
                        sideBGintensities = evalin('base',['sideBGintensities' signalName{r}]); % use of "signalName" (3.0)
                        cellBGintensities = evalin('base',['cellBGintensities' signalName{r}]);
                ...
                for r = 1:nRawImages
                    ...
                    sideBGintensities = evalin('base',['sideBGintensities' signalName{r}]);      % inverted these 2 lines
...
%                 SIDES.CATEGORIES = sideCATEGORIES;                                % STRUCTURE
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                sideSTATISTICS = SideStatistics(SIDES, signalName, cellCategoryTags); % 3.7
%                sideSTATISTICS = SideStatistics(SIDES, signalName);                          % use of signalName (3.1)
                ...
                CELLS.CategoryTags = cellCategoryTags;                      % 3.8                                          
                ...
                for r = 1:nRawImages
                    ...
                    cellSideIntensityDisorders = evalin('base',['cellSideIntensityDisorders' signalName{r}]);       % vector
                    ...
                    cellPolarityModes = evalin('base',['cellPolarityModes' signalName{r}]);                       % vector
...
%                 CELLS.CATEGORIES = cellCATEGORIES;                                                           % STRUCTURE
                ...
                % Resume filling up "allVertexCATEGORIES":
                allVertexCATEGORIES = SortVertices(VERTICES, SIDES, sideCATEGORIES); % 3.7
...
%                 VERTICES.CATEGORIES = allVertexCATEGORIES;
                ...
                %%% Extraction of side and allVertexCATEGORIES %%
                ...
                [coreRNs, FLRNs, borderRNs, nonBorderRNs] = GetCellCategories(cellCategoryTags); % 3.7
...
%                 CELLS.CATEGORIES = cellCATEGORIES;
                ...
                %%%% UPDATE OF "cellCategoryTags" IN "CELLS" (3.7):
                cellCategoryTags = MakeCellCategoryTags(cellCATEGORIES);
                CELLS.cellCategoryTags = cellCategoryTags;
...
%                 VERTICES.CATEGORIES = allVertexCATEGORIES;
            ...
            cmd = [SIAexe ...
                ' --binary "' segFilename '"' ...
                ' --channel "' [pathFolderRaw  filesep filenameRaw{1} num2str(n,digitsFormat) '.' imageFormatRaw] '"' ...
                ' --name ' signalName{1} ...
                ' --output "' fnBackupFile '"' ...
                ' --scale "' num2str(scale1D) '"' ...
                ];
            if nRawImages > 1
                for r = 1:nRawImages
                    cmd = [cmd ' --channel' num2str(r) ' "' [pathFolderRaw  filesep filenameRaw{r} num2str(n,digitsFormat) '.' imageFormatRaw] '"' ...
                        ' --name' num2str(r) ' ' signalName{r} ...
                        ];
                end
            end
            system(cmd);
            
            %% Saving backup file (3.6) %%
            % NB: backup already save by C++ app, but we modify it to fit better the Matlab SIA backup.
            disp(['Saving Backup file # ' fn]);
            load(fnBackupFile);    % load backup from C++           
            
            %%%% Extraction of data from saved structures into workspace with "ExtractData": (3.6)
            ExtractData(CELLS, 'cell');
            ExtractData(SIDES, 'side');
            ExtractData(VERTICES, 'allVertex');
            % Extracting cell categories from "cellCategoryTags" (3.7)
            [coreRNs, FLRNs, borderRNs, nonBorderRNs] = GetCellCategories(cellCategoryTags);
%             ExtractData(cellCATEGORIES);
        ...
        imageSize = size(segImage);                                        % updating image size in case not a real movie (Martial images) (3.8)
        clear imageCC; % 3.1
        fprintf('Done!\n')
        ...
        [coreRNs, FLRNs, borderRNs, nonBorderRNs] = GetCellCategories(cellCategoryTags);          % Extracting cell categories from "cellCategoryTags" (3.7)
        sideCATEGORIES = SortSides(sideNumbers, sideCells, cellCategoryTags); % 3.7
%         ExtractData(cellCATEGORIES);

        allVertexCATEGORIES = SortVertices(VERTICES, SIDES, sideCATEGORIES);
%         allVertexCATEGORIES = SortVertices(allVertexnCells, sideVertices, sideCATEGORIES);
        ExtractData(allVertexCATEGORIES);
...
%         imshow(segImage,'Border', 'tight')
        rawImage = evalin('base',['rawImage_' filenameRawMod{1}]);
        ...
        text(cellXsPixels(coreRNs),cellYsPixels(coreRNs), num2str(coreRNs), 'FontSize', fontSizeCellNumbers, 'HorizontalAlignment','center','Color','cyan','FontWeight', 'bold')
    ...
    for r = 1:nRawImages
        ...
        if displaySideIntensityDisorder == 1 && ~exist(thisFileName,'file')
            ...
            min_value = evalin('base',['min_cellSideIntensityDisorders' signalName{r} '_NBC']); % use of "signalName" (3.0)
            max_value = evalin('base',['max_cellSideIntensityDisorders' signalName{r} '_NBC']);
            mean_value = evalin('base',['mean_cellSideIntensityDisorders' signalName{r} '_NBC']);
            std_value = evalin('base',['std_cellSideIntensityDisorders' signalName{r} '_NBC']);
    ...
    for r = 1:nRawImages
        ...
        if displayPolarityMode == 1 && ~exist(thisFileName,'file')
            ...
            %%%% Loading:
            rawImage = evalin('base',['rawImage_' filenameRawMod{r}]);
            As = evalin('base',['cellPolarityModes' signalName{r}]); % using signalName (3.0), without "_" (3.0)
            ...
            thetaStar1s = - Phi1s;                                % NB: for angles in TRIGONOMETRIC CONVENTION, one should use theta_stars1s_CC = -theta_star1s = +Phi1s!!!!
            thetaStar2s = - Phi2s/2;                              % NB: for angles in TRIGONOMETRIC CONVENTION, one should use theta_stars2s_CC = -theta_star2s = +Phi2s/2!!!!
            ...
            %%%%% Mode 0+2 (2.1, 2.4.4, 3.4):
            if displayCellMode0
                nPlottedCells = length(plottedCells);
                for k = 1:nPlottedCells
                    kthCellPolarityEig1 = I0s(k) + I2s(k);                     % largest eigenvalue of 0th-2nd modes of intensity
                    kthCellPolarityEig2 = I0s(k) - I2s(k);                     % smallest eigenvalue
                    % 3.4
                    EDISPLAY.EdgeWidth = polarityLineWidth;
                    EDISPLAY.EdgeColor = mode2Color;
                    EDISPLAY.EdgeStyle = '-';
                    EDISPLAY.FaceOpacity = 0;
                    Ellipse(1/2*modeScale*kthCellPolarityEig1, 1/2*modeScale*kthCellPolarityEig2, XYsPixels(k,1), XYsPixels(k,2),thetaStar2s(k), EDISPLAY);
                    %                     Ellipse(1/2*modeScale*kthCellPolarityEig1, 1/2*modeScale*kthCellPolarityEig2, XYsPixels(k,1), XYsPixels(k,2),theta_star2s(k), mode2Color, polarityLineWidth)
            ...
            %%%%% Mode 1:
            if displayCellMode1
                quiver(XYsPixels(:,1), XYsPixels(:,2), modeScale*cos(deg2rad(thetaStar1s)).*I1s, modeScale*sin(deg2rad(thetaStar1s)).*I1s, 0, mode1Color,'MaxHeadSize',0.1);
                % NB: drawn after mode 2 so that arrows are above ellipses
            end
            
            %%%%% Mode 2:
            if displayCellMode2
                quiver(XYsPixels(:,1), XYsPixels(:,2), modeScale*1/2*cos(deg2rad(thetaStar2s)).*I2s, modeScale*1/2*sin(deg2rad(thetaStar2s)).*I2s, 0, mode2Color,'MaxHeadSize',0.001,'LineWidth',polarityLineWidth);
                quiver(XYsPixels(:,1), XYsPixels(:,2), -modeScale*1/2*cos(deg2rad(thetaStar2s)).*I2s, -modeScale*1/2*sin(deg2rad(thetaStar2s)).*I2s, 0, mode2Color,'MaxHeadSize',0.001,'LineWidth',polarityLineWidth);
    ...
    for r = 1:nRawImages
        ...
        if displayIntensity == 1 && ~exist(thisFileName,'file')
            ...
            % Averages of raw image r:
            min_value = evalin('base',['min_sideIntensities' signalName{r} '_NBS']); % using signalName (3.0)
            max_value = evalin('base',['max_sideIntensities' signalName{r} '_NBS']);
            mean_value = evalin('base',['mean_sideIntensities' signalName{r} '_NBS']);
            std_value = evalin('base',['std_sideIntensities' signalName{r} '_NBS']);
...
% 07/05/2018: 3.8
% - updated SIA Matlab part by defining and storing "cellCategoryTags".

% 03/05/2018: 3.7 (checked for C++SIA use)
% - stopped storing substructure "CATEGORIES" in CELLS,SIDES and VERTICES.
% - instead ONLY stores "CategoryTags" in CELLS and defines coreRNs, FLRNs,
% borderRNs with function "GetCellCategories".
% - in C++SIA, side & vertex numbers NO LONGER CORRESPOND TO THEIR ROW
% NUMBERS!! => requires extra steps to find their rows among all sides and
% vertices
% - accordingly updated "SortSides", "SortVertices", "CellStatistics",
% "SideStatistics", "CellDisplay"

% 15/03/2018: 3.5
% - updated "skeletonPixelsListPath" with new path to tracking outputs.

% 05/03/2018:
% - fixed display of polarity mode 0 involving "Ellipse".
