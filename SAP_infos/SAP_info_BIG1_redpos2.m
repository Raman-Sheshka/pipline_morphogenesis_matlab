...
finalFrame = 60;       % ends @ 60

%% Path to raw images %%

pathFolderRaw = 'C:\Users\ines\Documents\Python_Related\Evaluation_Stardist\BIGs\BIG1_1frame_outof3_pos2';
filenameRaw = {'BIG1_redpos2_'};
...
frameRef = 23;                              % frame # corresponding to "timeRef" determined by "TimeRegistration"
timeRef = '20h15';                          % "timeRef" ('HH:MM' or decimal) corresponding to "frameRef" above
dt = 15;                                     % time IN MINUTES between two frames
...
% xMacro1 = 389;                                       % 1st macro X coord clicked @26 hAPF
...
yMid = 95;                                 % y of midline IN PIXELS (leave empty [] if unknown)
xFactor = 0.864;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
yFactor = 0.871;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
boxSize = [40 40]/scale1D  ./ [xFactor yFactor];   % grid compartment size  IN MICRON. One value => square grid
% boxSize = [20 20]/scale1D ./ [xFactor yFactor];   % grid compartment size IN PIXELS [WIDTH HEIGHT]. One value => square grid
% boxSize = [128 128] ./ [xFactor yFactor];   % grid compartment size IN PIXELS [WIDTH HEIGHT]. One value => square grid
xyStart = [767 358];                        % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
