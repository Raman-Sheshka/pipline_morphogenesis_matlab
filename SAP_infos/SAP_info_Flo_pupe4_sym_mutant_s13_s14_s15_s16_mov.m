...
pathFolderRaw = 'D:\Florencia\Analysis_Florencia_Big_RhoGEF4\Flo_pupe4_sym_mutant_s13_s14_s15_s16_mov';
filenameRaw = {'Flo_pupe4_sym_mutant_s13_s14_s15_s16_mov_'};                  % can be multiple: {'pten_myo_' ; 'pten_baz_' ; ...}. NB: PIV will use FIRST listed
...
frameRef = 51;                              % frame # corresponding to "timeRef" determined by "TimeRegistration"
...
yMid = 156;                               % y of midline IN PIXELS (leave empty [] if unknown)
...
xFactor = 0.8855;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
yFactor = 0.8538;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
...
xyStart = [1197 743];                               % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
gridSize = [];                                      % number of compartments [ny nx]. If empty grid fills up image.
