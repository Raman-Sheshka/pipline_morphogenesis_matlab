...
pathFolderRaw = 'D:\Florencia\Analysis_Florencia_Big_RhoGEF4\Flo_pupe1_sym_s1_s2_s3_s4_mov';
filenameRaw = {'Flo_pupe1_sym_s1_s2_s3_s4_mov_'};                  % can be multiple: {'pten_myo_' ; 'pten_baz_' ; ...}. NB: PIV will use FIRST listed
...
frameRef = 60;                              % frame # corresponding to "timeRef" determined by "TimeRegistration"
...
yMid = 301;                               % y of midline IN PIXELS (leave empty [] if unknown)
...
xFactor = 0.9520;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
yFactor = 0.9053;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
...
xyStart = [1226 895];                               % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
gridSize = [];                                      % number of compartments [ny nx]. If empty grid fills up image.
