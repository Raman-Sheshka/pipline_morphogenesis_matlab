...
finalFrame = 181;       % ends @ 181

%% Path to raw images %%

pathFolderRaw = 'D:\Florencia\Analysis_Florencia_Big_RhoGEF4\Flo_pupe2_sym_mutant_s5_s6_s7_s8_mov';
filenameRaw = {'Flo_pupe2_sym_mutant_s5_s6_s7_s8_mov_'};                  % can be multiple: {'pten_myo_' ; 'pten_baz_' ; ...}. NB: PIV will use FIRST listed
...
yMid = 209;                               % y of midline IN PIXELS (leave empty [] if unknown)
...
xFactor = 1.0022;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
yFactor = 0.8273;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
...
xyStart = [1085 757];                               % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
gridSize = [];                                      % number of compartments [ny nx]. If empty grid fills up image.
