...
pathFolderRaw = 'E:\_Analyses_in_progress\Yohanns\wt_mov12_SW30';
filenameRaw = {'wt_mov12_DLcorr_'};                  % can be multiple: {'pten_myo_' ; 'pten_baz_' ; ...}. NB: PIV will use FIRST listed
...
frameRef = 53;                              % frame # corresponding to "timeRef" determined by "TimeRegistration"
...
yMid = 270;    % 1131 from ML clicks       % y of midline IN PIXELS (leave empty [] if unknown)
...
xFactor = 0.962;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
...
xyStart = [1177 796];                               % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
gridSize = [];                                      % number of compartments [ny nx]. If empty grid fills up image.
