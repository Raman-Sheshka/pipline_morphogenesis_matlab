...
finalFrame = 217;       % ends @ 217

%% Path to raw images %%

pathFolderRaw = 'C:\Users\ines\Documents\Python_Related\Evaluation_Stardist\BIGs\BIG6';  
filenameRaw = {'BIG6_'};      
...
frameRef = 71;                              % frame # corresponding to "timeRef" determined by "TimeRegistration"
...
% yMid = 102;                                 % y of midline IN PIXELS (leave empty [] if unknown)
% xFactor = 1.07;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
% yFactor = 1.02;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
% xMacro1 = 288;                                       % 1st macro X coord clicked @26 hAPF
...
yMid = 111;                                 % y of midline IN PIXELS (leave empty [] if unknown)
xFactor = 0.928;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
yFactor = 0.889;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
boxSize = [40 40]/scale1D  ./ [xFactor yFactor];   % grid compartment size IN  IN MICRON. One value => square grid
xyStart = [639 383];                        % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
gridSize = [];                              % number of compartments [ny nx]. If empty grid fills up image.

% % to fix Lorette's bug
% xFactor = 1.08;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
% yFactor = 1.02;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
% boxSize = [32,32]./[xFactor,yFactor]*(0.322./scale1D);
% xyStart = [638 384];                        % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
% gridSize = [];                              % number of compartments [ny nx]. If empty grid fills up image.
