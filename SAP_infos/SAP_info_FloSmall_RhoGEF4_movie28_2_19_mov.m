...
finalFrame = 193;       % ends @ 193

%% Path to raw images %%

pathFolderRaw = 'D:\Florencia\Analysis_Florencia\Flo_RhoGEF4_movie28_2_19_mov';
filenameRaw = {'RhoGEF4_movie28_2_19_mov_'};                  % can be multiple: {'pten_myo_' ; 'pten_baz_' ; ...}. NB: PIV will use FIRST listed
...
frameRef = 50;                              % frame # corresponding to "timeRef" determined by "TimeRegistration"
...
yMid = 109;                               % y of midline IN PIXELS (leave empty [] if unknown)
...
xFactor = 0.960;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
yFactor = 0.992;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
...
xyStart = [1079 632];                               % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
gridSize = [];                                      % number of compartments [ny nx]. If empty grid fills up image.
