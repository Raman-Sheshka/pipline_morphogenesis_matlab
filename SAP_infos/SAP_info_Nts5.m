...
finalFrame = 277;       % ends @ 277

%% Path to raw images %%

pathFolderRaw = 'D:\Lucie\Nts5';
filenameRaw = {'Nts5_'};                  % can be multiple: {'pten_myo_' ; 'pten_baz_' ; ...}. NB: PIV will use FIRST listed
...
frameRef = 48;                              % frame # corresponding to "timeRef" determined by "TimeRegistration"
...
yMid = [];    % 1131 from ML clicks       % y of midline IN PIXELS (leave empty [] if unknown)
...
boxSize = [1107 882];  	% grid compartment size IN PIXELS [WIDTH HEIGHT]. One value => square grid
xyStart = [435 765];                               % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
gridSize = [1 1];                                      % number of compartments [ny nx]. If empty grid fills up image.
