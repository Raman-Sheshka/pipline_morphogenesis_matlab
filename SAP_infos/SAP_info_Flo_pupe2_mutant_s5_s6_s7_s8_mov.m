...
pathFolderRaw = 'D:\Florencia\Analysis_Florencia_Big_RhoGEF4\Flo_pupe2_mutant_s5_s6_s7_s8_mov';
filenameRaw = {'Flo_pupe2_mutant_s5_s6_s7_s8_mov_'};                  % can be multiple: {'pten_myo_' ; 'pten_baz_' ; ...}. NB: PIV will use FIRST listed
...
yMid = 211;                               % y of midline IN PIXELS (leave empty [] if unknown)
...
xFactor = 0.9933;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
yFactor = 0.9159;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
...
xyStart = [1090 799];                               % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
gridSize = [];                                      % number of compartments [ny nx]. If empty grid fills up image.
