...
finalFrame = 200;       % ends @ 200

%% Path to raw images %%

pathFolderRaw = 'E:\_Analyses_in_progress\Yohanns\wt_N10_SW30';
filenameRaw = {'wt_N10_DLcorr_'};                  % can be multiple: {'pten_myo_' ; 'pten_baz_' ; ...}. NB: PIV will use FIRST listed
...
frameRef = 55;                              % frame # corresponding to "timeRef" determined by "TimeRegistration"
timeRef = '21h15';                          % "timeRef" ('HH:MM' or decimal) corresponding to "frameRef" above
dt = 5;                                     % time IN MINUTES between two frames
temperature = 29;                           % temperature (25 or 29C) at which development was filmed
yMid = 148;    % 1131 from ML clicks       % y of midline IN PIXELS (leave empty [] if unknown)
...
xFactor = 0.925;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
yFactor = 0.951;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
...
xyStart = [1466 714];                               % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
gridSize = [];                                      % number of compartments [ny nx]. If empty grid fills up image.
