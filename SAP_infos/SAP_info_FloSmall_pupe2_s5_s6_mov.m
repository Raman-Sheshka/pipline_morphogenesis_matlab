...
pathFolderRaw = 'D:\Florencia\Analysis_Florencia\Flo_pupe2_s5_s6_mov';
filenameRaw = {'Flo_pupe2_s5_s6_mov_'};                  % can be multiple: {'pten_myo_' ; 'pten_baz_' ; ...}. NB: PIV will use FIRST listed
...
yMid = 136;                               % y of midline IN PIXELS (leave empty [] if unknown)
...
xFactor = 0.965;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
yFactor = 0.925;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
...
xyStart = [1165 676];                               % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
gridSize = [];                                      % number of compartments [ny nx]. If empty grid fills up image.
