...
finalFrame = 177;       % ends @ 177

%% Path to raw images %%

pathFolderRaw = 'E:\_Analyses_in_progress\Yohanns\wt_mov13_SW30';
filenameRaw = {'wt_mov13_DLcorr_'};                  % can be multiple: {'pten_myo_' ; 'pten_baz_' ; ...}. NB: PIV will use FIRST listed
...
frameRef = 70;                              % frame # corresponding to "timeRef" determined by "TimeRegistration"
timeRef = '21h55';                          % "timeRef" ('HH:MM' or decimal) corresponding to "frameRef" above
dt = 5;                                     % time IN MINUTES between two frames
temperature = 29;                           % temperature (25 or 29C) at which development was filmed
yMid = 45;    % 1131 from ML clicks       % y of midline IN PIXELS (leave empty [] if unknown)
...
xFactor = 1.043;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
yFactor = 1.025;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
...
xyStart = [1158 572];                               % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
gridSize = [];                                      % number of compartments [ny nx]. If empty grid fills up image.

%For whiteRNAi before 'archetypisation'
% boxSize = [40 40]/scale1D  ./ [xFactor yFactor];  	% grid compartment size IN PIXELS [WIDTH HEIGHT]. One value => square grid
% xyStart = [717 1127];                               % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
% gridSize = [];                                      % number of compartments [ny nx]. If empty grid fills up image.
