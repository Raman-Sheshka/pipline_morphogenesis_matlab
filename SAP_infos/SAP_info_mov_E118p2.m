...
pathFolderRaw = 'D:\Olga\mov_E118p2';
filenameRaw = {'mov_E118p2_'};                  % can be multiple: {'pten_myo_' ; 'pten_baz_' ; ...}. NB: PIV will use FIRST listed
...
timeRef = '14h00';                          % "timeRef" ('HH:MM' or decimal) corresponding to "frameRef" above
...
% boxSize = [150 100]/ scale1D ./ [xFactor yFactor];   % grid compartment size IN PIXELS [WIDTH HEIGHT]. One value => square grid
% xyStart = [70 20];                        % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
% gridSize = [1 1]; 
% boxSize = [32 30]/ scale1D ./ [xFactor yFactor];   % grid compartment size IN PIXELS [WIDTH HEIGHT]. One value => square grid
% xyStart = [22 75];                        % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
% gridSize = [3 5];                              % number of compartments [ny nx]. If empty grid fills up image.
boxSize = [27 30]/ scale1D ./ [xFactor yFactor];   % grid compartment size IN PIXELS [WIDTH HEIGHT]. One value => square grid
xyStart = [88 40];                        % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
gridSize = [3 5];                              % number of compartments [ny nx]. If empty grid fills up image.
...
scaleBarLength = 10;                    % scale bar length in microns
