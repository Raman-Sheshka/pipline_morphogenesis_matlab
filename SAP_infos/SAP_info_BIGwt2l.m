...
pathFolderRaw = 'C:\Users\ines\Documents\Python_Related\Evaluation_Stardist\BIGs\BIGwt2l';   
filenameRaw = {'BIGwt2l_'};      
...
% yMid = 87;                                 % y of midline IN PIXELS (leave empty [] if unknown)
% xFactor = 1.01;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
% yFactor = 1.02;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
% xMacro1 = 334;                                       % 1st macro X coord clicked @26 hAPF
...
yMid = 82;                                 % y of midline IN PIXELS (leave empty [] if unknown)
xFactor = 0.878;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
yFactor = 0.863;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
boxSize = [40 40]/scale1D  ./ [xFactor yFactor];   % grid compartment size IN MICRON. One value => square grid
xyStart = [739 383];                        % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
