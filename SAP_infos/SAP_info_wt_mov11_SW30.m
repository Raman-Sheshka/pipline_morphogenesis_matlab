...
finalFrame = 176;       % ends @ 200

%% Path to raw images %%

pathFolderRaw = 'E:\_Analyses_in_progress\Yohanns\wt_mov11_SW30';
filenameRaw = {'wt_mov11_DLcorr_'};                  % can be multiple: {'pten_myo_' ; 'pten_baz_' ; ...}. NB: PIV will use FIRST listed
...
frameRef = 33;                              % frame # corresponding to "timeRef" determined by "TimeRegistration"
timeRef = '16h20';                          % "timeRef" ('HH:MM' or decimal) corresponding to "frameRef" above
dt = 5;                                     % time IN MINUTES between two frames
temperature = 29;                           % temperature (25 or 29C) at which development was filmed
yMid = 157;    % 1131 from ML clicks       % y of midline IN PIXELS (leave empty [] if unknown)
...
xFactor = 1.062;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
yFactor = 0.966;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
...
xyStart = [1174 642];                               % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
gridSize = [];                                      % number of compartments [ny nx]. If empty grid fills up image.
