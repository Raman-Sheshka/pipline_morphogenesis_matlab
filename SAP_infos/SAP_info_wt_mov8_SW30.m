...
pathFolderRaw = 'E:\_Analyses_in_progress\Yohanns\wt_mov8_SW30';
filenameRaw = {'wt_mov8_DLcorr_'};                  % can be multiple: {'pten_myo_' ; 'pten_baz_' ; ...}. NB: PIV will use FIRST listed
...
frameRef = 72;                              % frame # corresponding to "timeRef" determined by "TimeRegistration"
timeRef = '22h15';                          % "timeRef" ('HH f:MM' or decimal) corresponding to "frameRef" above
dt = 5;                                     % time IN MINUTES between two frames
temperature = 29;                           % temperature (25 or 29C) at which development was filmed
yMid = 252.5;                               % y of midline IN PIXELS (leave empty [] if unknown)
...
xFactor = 1.017;                                % X scaling factor from "RescaleAnimals". Animals LONGER than archetype: xFactor > 1
yFactor = 1.020;                                % Y scaling factor from "RescaleAnimals". Animals WIDER than archetype: yFactor > 1
...
xyStart = [1103 746];                               % xy coordinates (Upper Left Corner)of FIRST compartment. If empty, starts at image center.
gridSize = [];                                      % number of compartments [ny nx]. If empty grid fills up image.
